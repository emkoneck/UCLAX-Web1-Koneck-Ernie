...
•	Initial development begun by Tim Berners-Lee in 1980, before the development of the Internet, with a simple hypertext system called ENQUIRE
•	Berners-Lee published the initial HTML system, consisting of 18 elements, in 1991. Eleven of these elements are still used today. (Berners-Lee is widely credited as the inventor of the World Wide Web and created the first web site!)
•	HTML creates the structure of a document; developers use HTML coding to designate how a browser displays various elements of a page, such as text, headlines and images.
•	HTML primarily creates static web pages. Internet navigation between related pages and websites is achieved with HTML hyperlinks.
•	HTML uses tags which enclose different parts of content to make it appear a certain way (e.g., bold, italic, font size) or act a certain way (such as hyperlinking to another document or website.)

<h5>CSS – Cascading Style Sheets</h5>

•	Håkon Wium Lie and Bert Bos are considered the co-creators of CSS in 1995.
...
•	CSS allows the separation of the presentation of content from the structure (HTML), so a style sheet can be implemented across an entire website. Prior to CSS, if a developer wanted to assign a particular style to a h3 subhead, for example, HTML had to be used at each occurrence of that type of subhead.
•	CSS information can be embedded into an HTML document or can be a separate document.
•	CSS allows for greater control over the visual appearance of a site, with styles applied consistently sitewide.

<h5>JavaScript</h5>

•	Originally designed by Brendan Eich of Netscape in late 1995.
•	JavaScript is a scripting or programming language that governs how web pages behave. It’s used on 98% of all web pages.
•	The initial JavaScript standard that emerged was Microsoft’s Jscript, which was part of Internet Explorer-the leading web browser in the late 90’s and early 2000’s. By 2004, other browsers with more sophisticated scripting began to emerge, and by 2009 the worldwide standard became ECMAScript 5.
...
•	Server-side JavaScript generates new content on the server, such as data from a database.
•	Client-side JavaScript generates new content from inside the browser. For example, it can create a HTML table filled with data pulled from the server-side database, then display the table in a web page. This is common in e-commerce web sites.
•	APIs (Application Programming Interfaces) are sets of code that allow for the implementation of programs that would otherwise be difficult to implement. They are used to create such functionality as pop-up windows, animated graphics and interactive mapping functions.
...
Each user has a working copy of the code and the full change history. There is no single centralized version of the codebase.

Git differs from other version control systems in the way it thinks about data.

Most other systems store information as a list of file-based changes. Git basically takes a snapshot of what your files look like every time you commit and stores a reference to that snapshot.
...
JQuery and React are both JavaScript libraries that produce similar results, but use different techniques. JQuery simplifies the process of developing web apps using JavaScript, while React allows the embedding of HTML into Java Script.

<h5>DOM:</h5> JQuery directly interacts with the DOM; React uses virtual DOM, which improves speed and performance.
...
•	An anchor link is bound to a portion of a document, such as a heading or an image.
...
JPG is a universally recognized format that is compatible with most browsers, software and apps. Their biggest disadvantage is that lossy compression can result in posterization and digital artifacts that degrade the appearance of the image.
...
GIF files feature lossless compression, so when their data gets compressed, it doesn’t result in any loss of quality (unlike JPGs.) They are best used for images that aren’t extremely detailed.
...
PNG file sizes are generally much larger than JPGs or GIFs, so they take longer to open and transfer, and use more space on your computer’s hard drive. Also, they don’t support CMYK color, so transferring them to print can be problematic.
